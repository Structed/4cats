...
func run_all() -> void:
	...
	var ui_suite: GDScript = load("res://tools/test_ui_kit.gd")
	var ui_tests: RefCounted = ui_suite.new()
	_failures.append_array(ui_tests.run())
	var modal_suite: GDScript = load("res://tools/test_modal_host.gd")
	var modal_tests: RefCounted = modal_suite.new()
	_failures.append_array(modal_tests.run(get_tree()))
	var panel_suite: GDScript = load("res://tools/test_panels.gd")
	var panel_tests: RefCounted = panel_suite.new()
	_failures.append_array(panel_tests.run(get_tree()))
	var interaction_suite: GDScript = load("res://tools/test_interactions.gd")
	var interaction_tests: RefCounted = interaction_suite.new()
	_failures.append_array(interaction_tests.run())
	var section_suite: GDScript = load("res://tools/test_save_sections.gd")
	var section_tests: RefCounted = section_suite.new()
	_failures.append_array(section_tests.run())
	var translation_suite: GDScript = load("res://tools/test_translations.gd")
	var translation_tests: RefCounted = translation_suite.new()
	_failures.append_array(translation_tests.run())
